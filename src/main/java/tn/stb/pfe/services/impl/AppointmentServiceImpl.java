...
import tn.stb.pfe.exception.AppointmentNotFoundException;
import tn.stb.pfe.models.*;
...
import tn.stb.pfe.repositories.ChatMessageRepository;
...
public class AppointmentServiceImpl implements AppointmentService {
    ...
    public AppointmentServiceImpl(AppointmentRepository appointmentRepository, UserService userService, WorkService workService, ChatMessageRepository chatMessageRepository, NotificationService notificationService) {
    ...
    public Appointment getAppointmentById(int id) {
        return appointmentRepository.findById(id).orElseThrow( () -> new AppointmentNotFoundException("Appointment with id " + id + " not found"));
    }

    
    @Override
    public Appointment refuseAppointment(int appointmentId) {
    ...
    @Override
    public Appointment acceptAppointment(int appointmentId) {
        Appointment appointment = getAppointmentById(appointmentId);
    ...
    @Override
    public Appointment bookAppointment(int workId, int customerId, LocalDateTime start) {
    ...
    /**************************** */
        ...
        public void createNewAppointment(int workId, int providerId, int customerId, LocalDateTime start) {
            if (isAvailable(workId, providerId, customerId, start)) {
                ...
                appointment.setEnd(start.plusMinutes(work.getDuration()));
                appointmentRepository.save(appointment);
                notificationService.newNewAppointmentScheduledNotification(appointment);
            } else {
        ...
        public void addMessageToAppointmentChat(int appointmentId, int authorId,ChatMessage chatMessage) {
            ...
            if (appointment.getProvider().getId() == authorId || appointment.getCustomer().getId() == authorId) {
                ...
                notificationService.newChatMessageNotification(chatMessage);
            } 
        }
            ...
            appointmentRepository.findScheduledWithEndBeforeDate(LocalDateTime.now())
                    .forEach(appointment -> {
                        ...
                        if (LocalDateTime.now().minusDays(1).isBefore(appointment.getEnd())) {
                            notificationService.newAppointmentFinishedNotification(appointment);
                        }
                    });
        ...
        public void cancelUserAppointmentById(int appointmentId, int userId) {
            ...
            if (appointment.getCustomer().getId() == userId || appointment.getProvider().getId() == userId) {
                ...
                if (canceler.equals(appointment.getCustomer())) {
                    notificationService.newAppointmentCanceledByCustomerNotification(appointment);
                } else if (canceler.equals(appointment.getProvider())) {
                    notificationService.newAppointmentCanceledByProviderNotification(appointment);
                }
            }
        ...
        public boolean requestAppointmentRejection(int appointmentId, int customerId) {
            if (isCustomerAllowedToRejectAppointment(customerId, appointmentId)) {
                ...
                notificationService.newAppointmentRejectionRequestedNotification(appointment);
                updateAppointment(appointment);
                return true;
            } else {
                return false;
        ...
        public List<TimePeroid> excludeAppointmentsFromTimePeroids(List<TimePeroid> peroids,
            ...
            throw new UnsupportedOperationException("Unimplemented method 'excludeAppointmentsFromTimePeroids'");
        }
